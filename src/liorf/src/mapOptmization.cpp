...
public:
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            //gntmky
            //transformTobeMapped[0] = cloudInfo.imupitchinit;
            //transformTobeMapped[1] = cloudInfo.imurollinit;
            //gntmky
        ...
        if (cloudInfo.odomavailable == true)
        {
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                //gntmky
                //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "cloudInfo.imurollinit:%f, cloudInfo.imupitchinit:%f, cloudInfo.imuyawinit:%f",cloudInfo.imurollinit*180.0/3.14, cloudInfo.imupitchinit*180.0/3.14, cloudInfo.imuyawinit*180.0/3.14);
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    void addGPSFactor()
    {
        ...
        /*else
        {
            if (common_lib_->pointDistance(cloudKeyPoses3D->front(), cloudKeyPoses3D->back()) < 5.0){
                RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "pointDistance:%f",common_lib_->pointDistance(cloudKeyPoses3D->front(), cloudKeyPoses3D->back()));
        ...
        if (poseCovariance(3,3) < poseCovThreshold && poseCovariance(4,4) < poseCovThreshold){
            ...
            //gntmky
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "poseCovariance(3,3):%f,poseCovariance(4,4):%f < poseCovThreshold:%f",poseCovariance(3,3), poseCovariance(4,4), poseCovThreshold);
            //gntmky
        ...
        {
            if (ROS_TIME(gpsQueue.front().header.stamp) < timeLaserInfoCur - 0.2)  //- 115279.0
            {
                // message too old
                //gntmky
                //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "diff_laser-gps:%f",(timeLaserInfoCur - ROS_TIME(gpsQueue.front().header.stamp)));
                //gntmky

                gpsQueue.pop_front();
            }
            else if (ROS_TIME(gpsQueue.front().header.stamp) > timeLaserInfoCur + 0.2 ) //- 115279.0
            {
                // message too new
                //gntmky
                //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "message too new");
                //gntmky
            ...
            {
                //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "gps success!");
    ...
    void updatePath(const PointTypePose& pose_in)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "global_x:%f, global_y:%f, global_z:%f",pose_in.x,pose_in.y,pose_in.z);
    ...
    void publishOdometry()
    {
        ...
        //gntmky
        //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "global_r:%f, global_p:%f, global_y:%f",transformTobeMapped[0]*180.0/3.14, transformTobeMapped[1]*180.0/3.14, transformTobeMapped[2]*180.0/3.14);
        //gntmky
        geometry_msgs::msg::Quaternion quat_msg;
        tf2::convert(quat_tf, quat_msg);
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            //gntmky
            //quat_tf.setRPY(cloudInfo.imurollinit, cloudInfo.imupitchinit, cloudInfo.imuyawinit);
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "cloudInfo.imurollinit:%f, cloudInfo.imupitchinit:%f, cloudInfo.imuyawinit:%f",cloudInfo.imurollinit*180.0/3.14, cloudInfo.imupitchinit*180.0/3.14, cloudInfo.imuyawinit*180.0/3.14);
            //RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "roll:%f,pitch:%f, yaw:%f", roll*180.0/3.14, pitch*180.0/3.14, yaw*180.0/3.14);
